...
def rotor_roll_reverse(sequence, position):
    """This function simulate the rolling movement of a rotor in reverse
    Args:
        sequence: String, the actual rotor sequence
        position: Int, the number of movement the rotor as to do must be negative
    Return:
        string, the new sequence updated
    """
    sequence = sequence[position] + sequence[0:len(sequence) + position]
    return sequence
...
def encode_rotor(char, rotor_list):
    ...
    """
    #
    # rotor_list.index(char) return the index of the actual char in the rotor sequence
    #

    return alphabet[rotor_list.index(char)]
...
def encode_refractor(char, list_refractor):
    ...
    #
    # refractor.index(char) return the index of the actual char in the refractor sequence
    #

    return alphabet[list_refractor.index(char)]
...
def move_rotors_positions_reverse(_rotor_1, position_rotor_1,
                                  _rotor_2, position_rotor_2,
                                  _rotor_3, position_rotor_3):
    """Move the first rotor by -1, and check all potential revolution in reverse
    Args:
        _rotor_1 : string, current rotor_1 sequence
        position_rotor_1 : int, current state of the rotor
        _rotor_2 : string, current rotor_2 sequence
    ...
    position_rotor_1 -= 1
    _rotor_1 = rotor_roll_reverse(_rotor_1, -1)

    if position_rotor_1 == -1:

        position_rotor_1 = 25

        position_rotor_2 -= 1
        _rotor_2 = rotor_roll_reverse(_rotor_2, -1)
        if position_rotor_2 == -1:

            position_rotor_2 = 25

            position_rotor_3 -= 1
            _rotor_3 = rotor_roll_reverse(_rotor_3, -1)

            if position_rotor_3 == -1:
                position_rotor_3 = 25
...
def return_refractor(refractor):
    """Returns the associated string by the name of the refactor
    Args:
...
def get_input():
    ...
    message_encode = input("Enter the message to decode")
...
if __name__ == "__main__":
    ...
    #
    #   Note for decode : Understand the encode before read this
    #
    #   Here we are not encoding anymore, we need to get the real value of the encoded string to communicate,
    #   To do that, we need basically to do all step of enigma to encode the message but starting at the end
    #
    #   The rotor will be initialized as the initial position + length of string (convert as movement)
    #   And iterate on the encoded string from the end.
    #
    #   The main difference too for decode message, is in the rotor, rather than returns the char by the rotor sequence
    #   from his index in alphabet sequence rotor_list[alphabet.index(char)],decode message will set the rotor,to return
    #   the char by the alphabet sequence from his index in the alphabet sequence alphabet[rotor_list.index(char)]
    #   Same for refractor
    #   The rotation is after the decoding of the char
    #
    ...
    for i in range(0, len(message)):
        ...
        rotor_1, current_position_rotor_1, rotor_2, current_position_rotor_2, rotor_3, current_position_rotor_3 = \
            move_rotors_positions_reverse(rotor_1, current_position_rotor_1,
                                          rotor_2, current_position_rotor_2,
                                          rotor_3, current_position_rotor_3)
